...
The application illustrates the following techniques:
 ...
 Configuring and responding to selections in a tab bar //配置并响应tab bar的选择
 Displaying information in a tableview using both plain and grouped style table views//使用表格显示数据
 Using navigation controllers to navigate deeper into a data structure//使用导航控制器
 Subclassing UIView//子类化uiview
 Providing a custom UITableViewCell consisting of multiple subviews//子类化cell
 Implementing the UITableViewDelegate protocol//tableview 委托
 Implementing the UITableViewDataSource protocol//tableview 数据源 
 Reacting to taps in views //响应view中的点击
 Open a URL to an external web site using Safari //调用浏览器
 Flipping view content from front to back //旋转视图
 Creating a reflection of a view in the interface //在界面中创建一个view的倒影
